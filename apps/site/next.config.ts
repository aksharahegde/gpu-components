...
function stylexClientCssInjection(stylex: StylexPlugin) {
  ...
  return {
    apply(compiler: { hooks: { thisCompilation: { tap: Function } }; webpack: { Compilation: { PROCESS_ASSETS_STAGE_REPORT: number }; sources: { RawSource: new (s: string) => unknown } } }) {
      compiler.hooks.thisCompilation.tap('@stylexjs/unplugin/client-inject', (compilation: {
        ...
        compilation.hooks.processAssets.tap({ name: '@stylexjs/unplugin/client-inject', stage }, (assets: Record<string, unknown>) => {
