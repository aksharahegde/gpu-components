...
export class GraphComponent implements GpuComponent<GraphProps> {
  ...
  /**
   * `create()` re-uploads the original seed positions after device-loss recovery, but that alone
   * leaves `iterations` stale: a graph that had already converged (`iterations >= MAX_ITERATIONS`,
   * `animating = false`) would come back with fresh seed positions and no simulation to move them
   * — frozen forever, since nothing re-arms `animating`. Resetting both here re-runs the layout
   * from the recovered seed exactly like a fresh mount would.
   */
