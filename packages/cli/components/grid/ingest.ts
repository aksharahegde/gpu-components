...
export interface GridData {
   ...
   */
  /** `<ArrayBuffer>` explicitly — written straight to a `StorageBuffer` (see the heatmap note). */
...
export function computeColumnRanges(
  numeric: Float32Array<ArrayBuffer>,
  rowCount: number,
...
export function ingestRows(rows: readonly RawRow[], columns: readonly GridColumn[]): GridData {
  ...
  const numeric = new Float32Array(new ArrayBuffer(rowCount * Math.max(1, numericColumnCount) * 4));
