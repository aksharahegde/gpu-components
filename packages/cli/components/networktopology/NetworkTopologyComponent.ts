...
export class NetworkTopologyComponent implements GpuComponent<TopologyProps> {
  ...
  /**
   * `create()` re-uploads the original seed positions after device-loss recovery. Unlike
   * `GraphComponent`, `animating` here already tracks `!paused` alone (see class doc), so it needs
   * no correction — but `iterations` would otherwise stay stale at its pre-loss value, leaving a
   * converged layout's recovered seed positions un-simulated. Resetting it re-runs the layout pass
   * from the recovered seed, same as a fresh mount.
   */
